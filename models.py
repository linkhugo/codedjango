...
class HealthCheckF5(models.Model):
    """
    Snapshot diario de salud de un appliance F5 BIG-IP.
    Unique constraint en (fqdn, fecha). Campos clave: failover, sync, cpu_usage, file_backup.
    """
...
class LDAPGroupMap(models.Model):
    """
    Mapea un DN contenedor de LDAP a un Group de Django.
    El backend LDAP lee esta tabla en cada login para construir templates de DN,
    asignar el grupo y otorgar is_staff/is_superuser.
    """
...
class LDAPConfig(models.Model):
    """
    Configuración de conexión al servidor LDAP, gestionada desde Admin.
    El backend lee el primer registro activo en cada login.
    """
