...
from .models import (
    Company, Datacenter, LBPhysical, LBGuest,
    F5SoftwareVersion, F5HardwareModel,
    HealthRule, VIP, DocEntry, DirectoryEntry,
    CredentialRotationPolicy, CredentialRotation,
...
class LBPhysicalForm(forms.ModelForm):
    ...
    class Meta:
        model = LBPhysical
        fields = [
            'device', 'ci_id', 'mgmt_ip', 'f5_version', 'distro', 'f5_model',
        ...
        widgets = {
            ...
            'f5_version':    forms.Select(attrs={'class': 'form-select'}),
            'distro':        forms.TextInput(attrs={'class': 'form-control', 'data-autocomplete': 'lb_physical,distro'}),
            'f5_model':      forms.Select(attrs={'class': 'form-select'}),
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Required fields (f5_version/f5_model replace version/model as required)
        for field in ('mgmt_ip', 'f5_version', 'distro', 'f5_model', 'serial',
    ...
    def save(self, commit: bool = True) -> LBPhysical:
        """Sync version/model CharFields from the selected FK entries."""
        instance = super().save(commit=False)
        if instance.f5_version:
            instance.version = instance.f5_version.version
        if instance.f5_model:
            instance.model = instance.f5_model.name
        if commit:
            instance.save()
        return instance
...
class LBGuestForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # Required fields (f5_version replaces version; f5_model optional for VMs)
        for field in ('device', 'mgmt_ip', 'f5_version', 'company'):
            self.fields[field].required = True
        # Device is not editable after creation
        if self.instance and self.instance.pk:
            self.fields['device'].disabled = True
    ...
    def save(self, commit: bool = True) -> LBGuest:
        """Sync version/model CharFields from the selected FK entries."""
        instance = super().save(commit=False)
        if instance.f5_version:
            instance.version = instance.f5_version.version
    ...
    class Meta:
        ...
        widgets = {
            ...
            'category':    forms.TextInput(attrs={
                'class': 'form-control',
                'placeholder': 'Procedimientos, Diseño, Proveedores…',
            }),
...
class F5SoftwareVersionForm(forms.ModelForm):
    """
    Form for creating or editing an F5 software version catalog entry.

    Lifecycle fields:
      - eosd_date: End of Software Development (F5 stops releasing patches)
      - eots_date: End of Technical Support    (F5 stops providing tech support)
    """

    class Meta:
        model  = F5SoftwareVersion
        fields = ['version', 'eosd_date', 'eots_date', 'notes']
        widgets = {
            'version':   forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'ej. 17.5.1'}),
            'eosd_date': forms.DateInput(attrs={'class': 'form-control', 'type': 'date'}, format='%Y-%m-%d'),
            'eots_date': forms.DateInput(attrs={'class': 'form-control', 'type': 'date'}, format='%Y-%m-%d'),
            'notes':     forms.Textarea(attrs={'class': 'form-control', 'rows': 2,
                                               'placeholder': 'Notas opcionales'}),
        }
        labels = {
            'eosd_date': 'EoSD — End of Software Development',
            'eots_date': 'EoTS — End of Technical Support',
        }
...
class F5HardwareModelForm(forms.ModelForm):
    """
    Form for creating or editing an F5 hardware model catalog entry.

    Lifecycle fields:
      - eoss_date: End of Software Support  (F5 stops releasing software for this hardware)
      - eots_date: End of Technical Support (F5 stops providing tech support)
      - eol_tbd:   Check when vendor has not yet published the EoL date
    """

    class Meta:
        model  = F5HardwareModel
        fields = ['name', 'eoss_date', 'eots_date', 'eol_tbd', 'notes']
        widgets = {
            'name':      forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'ej. i5800'}),
            'eoss_date': forms.DateInput(attrs={'class': 'form-control', 'type': 'date'}, format='%Y-%m-%d'),
            'eots_date': forms.DateInput(attrs={'class': 'form-control', 'type': 'date'}, format='%Y-%m-%d'),
            'eol_tbd':   forms.CheckboxInput(attrs={'class': 'form-check-input'}),
            'notes':     forms.Textarea(attrs={'class': 'form-control', 'rows': 2,
                                               'placeholder': 'Notas opcionales'}),
        }
        labels = {
            'eoss_date': 'EoSS — End of Software Support',
            'eots_date': 'EoTS — End of Technical Support',
            'eol_tbd':   'Sin fecha de finalización (el vendor aún no la publicó)',
        }
...
class CredentialRotationPolicyForm(forms.ModelForm):
    """Form for creating or editing a CredentialRotationPolicy."""

    class Meta:
        model  = CredentialRotationPolicy
        fields = ['name', 'days_interval', 'warn_days_before']
        widgets = {
            'name':             forms.TextInput(attrs={'class': 'form-control',
                                                       'placeholder': 'Ej. Producción 90 días'}),
            'days_interval':    forms.NumberInput(attrs={'class': 'form-control', 'min': 1}),
            'warn_days_before': forms.NumberInput(attrs={'class': 'form-control', 'min': 1}),
        }


class CredentialRotationForm(forms.ModelForm):
    """Form for creating or editing a CredentialRotation group entry."""

    class Meta:
        model  = CredentialRotation
        fields = ['device_type', 'credential_type', 'environment', 'policy', 'ticket_ref', 'notes']
        widgets = {
            ...
            'ticket_ref':      forms.TextInput(attrs={'class': 'form-control',
        ...
        labels = {
            ...
            'environment':     'Entorno',
            'policy':          'Política de rotación',
            'ticket_ref':      'Ticket de cambio',
            'notes':           'Notas',
        }
...
class LBChangeTemplateForm(forms.ModelForm):
    """Form for creating or editing an LBChangeTemplate."""

    class Meta:
        model  = LBChangeTemplate
        fields = ['name', 'change_type', 'description', 'active']
        widgets = {
            'name':        forms.TextInput(attrs={'class': 'form-control',
                                                   'placeholder': 'Ej. F5 Software Upgrade'}),
...
class LBChangeTemplateItemForm(forms.ModelForm):
    """
    Form for creating or editing a single LBChangeTemplateItem.

    Args:
        template: LBChangeTemplate instance used to filter linked_item queryset.
    """

    class Meta:
        model  = LBChangeTemplateItem
        fields = ['phase', 'order', 'text', 'required', 'comment_required_on', 'linked_item']
        widgets = {
            'phase':               forms.Select(attrs={'class': 'form-select form-select-sm'}),
            ...
            'comment_required_on': forms.Select(attrs={'class': 'form-select form-select-sm'}),
            'linked_item':         forms.Select(attrs={'class': 'form-select form-select-sm'}),
        }
        labels = {
            'phase':               'Fase',
            'order':               'Orden',
            'text':                'Texto de verificación',
            'required':            'Obligatorio',
            'comment_required_on': 'Pedir comentario',
            'linked_item':         'Ítem vinculado (contexto)',
        }

    def __init__(self, *args, template=None, **kwargs):
        super().__init__(*args, **kwargs)
        qs = LBChangeTemplateItem.objects.none()
        if template:
            qs = LBChangeTemplateItem.objects.filter(template=template).order_by('phase', 'order')
            if self.instance.pk:
                qs = qs.exclude(pk=self.instance.pk)
        self.fields['linked_item'].queryset  = qs
        self.fields['linked_item'].required  = False
        self.fields['linked_item'].empty_label = '— Sin vínculo —'
        self.fields['comment_required_on'].choices = [('', '— Nunca —')] + list(LBChangeTemplateItem.COMMENT_ON)


class LBChangeRequestForm(forms.ModelForm):
    """Form for creating or editing an LBChangeRequest."""

    class Meta:
        model  = LBChangeRequest
        fields = [
            'title', 'template', 'lb_physical', 'lb_guest',
            'from_version', 'target_version', 'ticket_ref',
            'scheduled_at', 'notes',
        ]
        widgets = {
            'title':          forms.TextInput(attrs={'class': 'form-control',
                                                     'placeholder': 'Ej. Upgrade f5-prod-01 17.1.1→17.1.2'}),
            'template':       forms.Select(attrs={'class': 'form-select'}),
            'lb_physical':    forms.Select(attrs={'class': 'form-select'}),
            'lb_guest':       forms.Select(attrs={'class': 'form-select'}),
            'from_version':   forms.TextInput(attrs={'class': 'form-control',
                                                     'placeholder': 'Versión actual'}),
            'target_version': forms.Select(attrs={'class': 'form-select'}),
            'ticket_ref':     forms.TextInput(attrs={'class': 'form-control',
                                                     'placeholder': 'CHG0012345'}),
            'scheduled_at':   forms.DateTimeInput(attrs={'class': 'form-control',
                                                          'type': 'datetime-local'},
                                                   format='%Y-%m-%dT%H:%M'),
        ...
        labels = {
            ...
            'template':       'Plantilla de checklist',
            'lb_physical':    'LB Physical (si aplica)',
            'lb_guest':       'LB Guest (si aplica)',
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields['template'].queryset = LBChangeTemplate.objects.filter(active=True)
        self.fields['scheduled_at'].input_formats = ['%Y-%m-%dT%H:%M']

    def clean(self):
        """Validate that at most one device is selected."""
        cleaned = super().clean()
        phys  = cleaned.get('lb_physical')
        guest = cleaned.get('lb_guest')
        if phys and guest:
            raise forms.ValidationError(
                'Selecciona solo un dispositivo: LB Physical o LB Guest, no ambos.'
            )
        return cleaned
