...
def bitacora_hardening_bulk_action(request):
    ...
    if action == 'close':
        codes    = list(qs.values_list('code', flat=True).distinct())
        assigned = list(qs.values_list('assigned_user_id', flat=True).distinct())
        if len(codes) != 1:
            return JsonResponse({
                'ok': False,
        ...
        if len(assigned) != 1 or assigned[0] is None:
            return JsonResponse({
                'ok': False,
                'error': 'Todos los tickets deben tener el mismo usuario asignado (y no pueden estar sin asignar).',
            })
        updated = qs.exclude(status=BitacoraHardening.Status.CLOSED).update(
