"""
insert_vips_django.py
Sincroniza VIPs desde JSON hacia Django_DEV via ORM.
...
PATH_JSON_FILES = Path("/var/lb/vips")
...
JSON_FIELDS = {"profiles"}  # JSONField — pasar Python directo
...
def validate_vip(raw: dict) -> dict:
    """
    ...
    if not data.get("full_path") or not data.get("ltm_fqdn"):
        raise ValueError(
            f"VIP sin full_path o ltm_fqdn: "
            f"full_path={data.get('full_path')!r}, ltm_fqdn={data.get('ltm_fqdn')!r}"
        )
    return data


def upsert_vip(data: dict) -> tuple[VIP, bool]:
    """
    ...
    return VIP.objects.update_or_create(**lookup, defaults=defaults)


def process_json_file(file_path: Path) -> tuple[int, int]:
    """Procesa un archivo JSON. Retorna (insertados, actualizados)."""
    try:
        vips = json.loads(file_path.read_text(encoding="utf-8"))
    ...
    for raw in vips:
        try:
            data = validate_vip(raw)
            _, created = upsert_vip(data)
            if created:
                inserted += 1
            else:
                updated += 1
        except ValueError as exc:
            logging.warning("VIP omitido en '%s': %s", file_path.name, exc)
