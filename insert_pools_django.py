"""
insert_pools_django.py
Sincroniza Pools desde JSON hacia Django_DEV via ORM.
...
from lb_manager.models import Pool
...
PATH_JSON_FILES = Path("/var/lb/pools")

# ── Constantes de validación derivadas del modelo ────────────────────────────
INT_FIELDS = {"active_member_count", "available_member_count", "member_count"}
JSON_FIELDS = {"members", "monitors"}  # JSONField — pasar Python directo
MAX_LENGTHS = {
    "allow_nat":           10,
    "allow_snat":          10,
    "client_ip_tos":       50,
...
POOL_FIELDS = list(MAX_LENGTHS.keys()) + list(INT_FIELDS) + list(JSON_FIELDS)
...
def validate_pool(raw: dict) -> dict:
    """
    ...
    if not data.get("full_path") or not data.get("ltm_fqdn"):
        raise ValueError(
            f"Pool sin full_path o ltm_fqdn: "
            f"full_path={data.get('full_path')!r}, ltm_fqdn={data.get('ltm_fqdn')!r}"
        )
    return data


def upsert_pool(data: dict) -> tuple[Pool, bool]:
    """
    ...
    return Pool.objects.update_or_create(**lookup, defaults=defaults)


def process_json_file(file_path: Path) -> tuple[int, int]:
    """Procesa un archivo JSON. Retorna (insertados, actualizados)."""
    try:
        pools = json.loads(file_path.read_text(encoding="utf-8"))
    ...
    for raw in pools:
        try:
            data = validate_pool(raw)
            _, created = upsert_pool(data)
            if created:
                inserted += 1
            else:
                updated += 1
        except ValueError as exc:
            logging.warning("Pool omitido en '%s': %s", file_path.name, exc)
