...
from .utils import _dt_length, _unassigned_pools_qs, _unassigned_nodes_qs, _ha_group
...
def servicio_sync(request: 'HttpRequest') -> JsonResponse:
    ...
    # ── Construir mapa HA bidireccional desde inventario ─────────────────────
    ha_map: dict[str, str] = {}
    ha_map.update(
        LBPhysical.objects.filter(ha_pair__isnull=False).exclude(ha_pair='')
        .values_list('device', 'ha_pair')
    ...
    # Bidireccional: ltm01→ltm02 implica ltm02→ltm01
    ha_map.update({v: k for k, v in list(ha_map.items()) if v not in ha_map})
...
def servicio_delete_no_vip(request: 'HttpRequest') -> 'HttpResponse':
    ...
    (mismo name + ltm_fqdn). Si se recibe ltm_fqdn en el POST, solo borra
    los registros de ese equipo (y su HA partner).
    ...
    has_vip = VIP.objects.filter(
        name=OuterRef('name'),
        ltm_fqdn=OuterRef('ltm_fqn'),
    )
    qs = Servicio.objects.annotate(has_vip=Exists(has_vip)).filter(has_vip=False)
