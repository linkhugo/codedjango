...
def main() -> None:
    if len(sys.argv) >= 2 and not re.match(r'^\d{4}-\d{2}-\d{2}$', sys.argv[1].strip()):
        # Argumento es un path completo
        file_path = Path(sys.argv[1].strip())
    else:
        # Sin argumento o argumento es solo una fecha — siempre usar localdate()
        if len(sys.argv) >= 2:
            logging.warning("Argumento '%s' ignorado — usando localdate()", sys.argv[1].strip())
        today     = localdate()
        logging.info("Fecha local (localdate): %s", today)
        fecha_str = today.strftime("%Y-%m-%d")
        file_path = PATH_CSV / f"health_check_{fecha_str}.csv"

    logging.info("Buscando archivo: %s", file_path.resolve())

    if not file_path.is_file():
        # Listar archivos disponibles en el directorio para diagnóstico
        parent = file_path.parent
        if parent.is_dir():
            archivos = sorted(f.name for f in parent.iterdir() if f.is_file())
            logging.error("Archivo no encontrado: %s", file_path.resolve())
            logging.error("Archivos disponibles en %s: %s", parent, archivos or "ninguno")
        else:
            logging.error("Directorio no existe: %s", parent)
        sys.exit(1)
