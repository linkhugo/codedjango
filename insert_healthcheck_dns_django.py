...
def main() -> None:
    if len(sys.argv) >= 2:
        ...
        # Si el argumento es solo una fecha YYYY-MM-DD, construir el path completo
        if re.match(r'^\d{4}-\d{2}-\d{2}$', arg):
            logging.info("Argumento detectado como fecha: %s", arg)
            file_path = PATH_CSV / f"health_check_{arg}.csv"
        else:
            file_path = Path(arg)
    else:
        today     = localdate()
