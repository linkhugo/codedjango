"""
insert_client_ssl_profiles_django.py
Sincroniza perfiles SSL cliente desde JSON hacia Django_DEV via ORM.
...
from lb_manager.models import ClientSSLProfile
...
PATH_JSON_FILES = Path("/var/lb/client_ssl_profiles")

# ── Constantes de validación derivadas del modelo ────────────────────────────
INT_FIELDS = {"authenticate_depth", "cache_size", "cache_timeout"}
MAX_LENGTHS = {
    "allow_non_ssl": 10,
    "profile_mode_enabled": 10,
    "renegotiation": 10,
    "retain_certificate": 10,
    "session_ticket": 10,
    "sni_default": 10,
    "strict_name": 10,
    "authenticate_frequency": 50,
    "peer_certificate_mode": 50,
    "secure_renegotiation_mode": 50,
    "certificate_file": 255,
    "chain_file": 255,
    "description": 255,
    "full_path": 255,
    "name": 255,
...
PROFILE_FIELDS = list(MAX_LENGTHS.keys()) + list(INT_FIELDS)
...
def validate_profile(raw: dict) -> dict:
    ...
    if not data.get("full_path") or not data.get("ltm_fqdn"):
        raise ValueError(
            f"Perfil sin full_path o ltm_fqdn: "
            f"full_path={data.get('full_path')!r}, ltm_fqdn={data.get('ltm_fqdn')!r}"
        )
    return data


def upsert_profile(data: dict) -> tuple[ClientSSLProfile, bool]:
    """
    ...
    return ClientSSLProfile.objects.update_or_create(**lookup, defaults=defaults)


def process_json_file(file_path: Path) -> tuple[int, int]:
    """Procesa un archivo JSON. Retorna (insertados, actualizados)."""
    try:
        profiles = json.loads(file_path.read_text(encoding="utf-8"))
    ...
    for raw in profiles:
        try:
            data = validate_profile(raw)
            _, created = upsert_profile(data)
            if created:
                inserted += 1
            else:
                updated += 1
        except ValueError as exc:
            logging.warning("Perfil omitido en '%s': %s", file_path.name, exc)
