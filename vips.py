...
from .utils import _dt_params, _excel_headers, _ha_group, apply_search_filter


class VIPUpdateView(CRUDContextMixin, LoginRequiredMixin, PermissionRequiredMixin, UpdateView):
    """
    ...
    crud_list_url   = 'vip_list'
    crud_list_label = 'VIPs'
...
def vip_data(request):
    ...
    qs = apply_search_filter(qs, search, [
        'name', 'destination_address', 'destination_port', 'protocol', 'type',
        'availability_status', 'default_pool', 'snat_type', 'source_address',
        'profiles', 'description', 'policies', 'ltm_fqdn',
    ])
    filtered = qs.count()
...
class VIPDormantListView(LoginRequiredMixin, PermissionRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs: object) -> dict:
        """
        Agrega ``device_groups`` (pares HA agrupados) y el total de VIPs dormant
        al contexto. Usa SiteSettings.decommission_lookback_months como ventana.
        """
        ctx = super().get_context_data(**kwargs)
                ...
                JOIN (
                    ...
                    HAVING COUNT(*) > 0
                       AND COUNT(*) = COUNT(CASE
                           WHEN LOWER(enabled) = 'no'
                               OR (LOWER(enabled) = 'yes' AND LOWER(availability_status) = 'offline')
                           THEN 1 END)
...
def vip_dormant_data(request):
    ...
    cfg        = SiteSettings.objects.first()
    lookback   = cfg.decommission_lookback_months if cfg else 3
    draw, start, length, search, col_idx, col_dir = _dt_params(
        request, cfg, default_col=8, default_dir='desc',
    )
    ...
    base_sql = f"""
        ...
        FROM vips v
        JOIN (
            -- ALL snapshots in the last N months must be inactive:
            -- enabled='no'  OR  (enabled='yes' AND availability_status='offline')
            SELECT name, ltm_fqdn
            FROM lb_vips_historical
            WHERE date >= NOW() - (INTERVAL '1 month' * %s)
            GROUP BY name, ltm_fqdn
...
class LBVIPHistoricalListView(LoginRequiredMixin, PermissionRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs: object) -> dict:
        """
        Agrega ``device_groups`` (pares HA agrupados desde LBPhysical/LBGuest)
        y ``ha_map_json`` al contexto, para el dropdown HA-aware del filtro LTM.
        """
        ctx = super().get_context_data(**kwargs)
        ...
        fqdns = list(
            LBVIPHistorical.objects.exclude(ltm_fqdn__isnull=True).exclude(ltm_fqdn='')
            .values_list('ltm_fqdn', flat=True).distinct().order_by('ltm_fqdn')
        )
        ha_map: dict[str, str] = {}
        ha_map.update(
...
def lb_vip_historical_data(request):
    ...
    draw, start, length, _, col_idx, col_dir = _dt_params(
        request, default_col=19, default_dir='desc',
    )
    ...
    if f_ltm:
        qs = qs.filter(ltm_fqdn__in=_ha_group(f_ltm))
