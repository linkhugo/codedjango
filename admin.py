...
from .models import (
    Company, Datacenter, LBPhysical, LBGuest, LBDecommissioned, Servicio,
...
class ReadOnlyAdminMixin:
    """
    Disallows creation and modification of records via the admin.

    Used for immutable history/audit models (decommission log, rotation
    events, login audit). Deletion is still permitted unless the subclass
    overrides ``has_delete_permission`` — keeps the door open for purging
    old data.

    Must be listed BEFORE ``UnfoldModelAdmin`` in the class declaration so
    the mixin's permission methods take precedence in the MRO.
    """
...
class SingletonAdminMixin:
    """
    Allows adding only the first record of a model (singleton config).

    Used for global-configuration models (SiteSettings, LDAPConfig) where
    a second row would be confusing or harmful. Uses ``self.model`` which
    Django auto-sets when the admin is registered via ``@admin.register``.

    Must be listed BEFORE ``UnfoldModelAdmin`` so ``has_add_permission``
    takes precedence.
    """
...
class LBChangeTemplateItemInline(admin.TabularInline):
    model  = LBChangeTemplateItem
    extra  = 2
    fields = ('phase', 'order', 'text', 'required')


@admin.register(LBChangeTemplate)
class LBChangeTemplateAdmin(UnfoldModelAdmin):
    """Checklist templates for LB upgrades and migrations."""

    icon          = 'checklist'
    list_display  = ('name', 'change_type', 'active')
    list_filter   = ('change_type', 'active')
    search_fields = ('name', 'description')
    inlines       = [LBChangeTemplateItemInline]


@admin.register(LBChangeRequest)
class LBChangeRequestAdmin(UnfoldModelAdmin):
    """LB change instances with pre/post checklist signatures."""

    icon          = 'pending_actions'
    list_display  = ('pk', 'title', 'ticket_ref', 'lb_physical', 'lb_guest',
                     'status', 'scheduled_at', 'pre_signed_by', 'post_signed_by',
                     'postmortem_had_issues')
    list_filter   = ('status', 'template__change_type', 'postmortem_had_issues')
    search_fields = ('title', 'ticket_ref', 'lb_physical__device', 'lb_guest__device',
                     'postmortem_notes')
    list_select_related = ['template', 'lb_physical', 'lb_guest',
                           'pre_signed_by', 'post_signed_by', 'postmortem_filled_by']
    ordering      = ('-created_at',)
    fieldsets     = (
        (None, {
            'fields': ('title', 'template', 'lb_physical', 'lb_guest',
                       'from_version', 'target_version', 'ticket_ref',
                       'scheduled_at', 'status', 'notes', 'created_by'),
        }),
        ('Firmas', {
            'fields': ('initial_signed_by', 'initial_signed_at',
                       'pre_signed_by', 'pre_signed_at',
                       'post_signed_by', 'post_signed_at'),
            'classes': ('collapse',),
        }),
        ('Post-Mortem', {
            'fields': ('postmortem_had_issues', 'postmortem_notes',
                       'postmortem_filled_by', 'postmortem_filled_at'),
            'classes': ('collapse',),
        }),
    )
...
@admin.register(F5Meeting)
class F5MeetingAdmin(UnfoldModelAdmin):
    """Reuniones mensuales de revisión F5."""

    icon          = 'handshake'
    list_display  = ('id', 'title', 'meeting_date', 'created_by', 'agreement_count')
    ...
    @unfold_display(description='Acuerdos')
    def agreement_count(self, obj):
        return obj.agreements.count()


@admin.register(F5Agreement)
class F5AgreementAdmin(UnfoldModelAdmin):
    """Acuerdos derivados de las reuniones F5."""

    icon          = 'task_alt'
    list_display  = ('id', 'title', 'meeting', 'owner', 'due_date',
                     'priority_badge', 'status_badge', 'progress_pct')
    list_filter   = ('status', 'priority', 'due_date')
    search_fields = ('title', 'description', 'closure_notes',
                     'owner__username', 'meeting__title')
    list_select_related = ['meeting', 'owner', 'created_by', 'closed_by']
    ordering      = ('due_date', '-created_at')

    @unfold_display(description='Prioridad', label={
        'LOW': 'info', 'MEDIUM': 'warning', 'HIGH': 'danger',
    })
    def priority_badge(self, obj):
        return obj.priority

    @unfold_display(description='Estado', label={
        'OPEN': 'info', 'IN_PROGRESS': 'warning',
        'DONE': 'success', 'BLOCKED': 'danger', 'CANCELLED': 'secondary',
    })
    def status_badge(self, obj):
        return obj.status


@admin.register(CredentialRotationPolicy)
class CredentialRotationPolicyAdmin(UnfoldModelAdmin):
    """Policies that define rotation interval and warning period."""

    icon          = 'policy'
    list_display  = ('name', 'days_interval', 'warn_days_before')
    search_fields = ('name',)
    ordering      = ('name',)


@admin.register(CredentialRotationEvent)
class CredentialRotationEventAdmin(ReadOnlyAdminMixin, UnfoldModelAdmin):
    """Immutable audit log of credential rotation events."""

    icon          = 'history'
    list_display  = ('rotation', 'rotated_at', 'rotated_by', 'ticket_ref')
    list_filter   = ('rotation__device_type', 'rotation__credential_type')
    search_fields = ('ticket_ref', 'notes', 'rotated_by__username')
    list_select_related = ['rotation', 'rotated_by']
    ordering      = ('-rotated_at',)


@admin.register(CredentialRotation)
class CredentialRotationAdmin(UnfoldModelAdmin):
    """Rotation metadata per device group. Never stores passwords."""

    icon          = 'key'
    list_display  = ('device_type', 'credential_type', 'policy', 'last_rotated', 'rotated_by', 'ticket_ref')
    list_filter   = ('device_type', 'credential_type', 'policy')
    search_fields = ('ticket_ref', 'notes')
    list_select_related = ['policy', 'rotated_by']
    ordering      = ('device_type', 'credential_type')


@admin.register(F5SoftwareVersion)
class F5SoftwareVersionAdmin(UnfoldModelAdmin):
    """Catalog of F5 BIG-IP software versions with EoSD/EoTS dates."""

    icon          = 'code'
    list_display  = ('version', 'eosd_date', 'eots_date', 'notes')
    search_fields = ('version', 'notes')
    list_filter   = ()
    ordering      = ('version',)


@admin.register(F5HardwareModel)
class F5HardwareModelAdmin(UnfoldModelAdmin):
    """Catalog of F5 BIG-IP hardware models with EoSS/EoTS dates and TBD flag."""

    icon          = 'memory'
    list_display  = ('name', 'eoss_date', 'eots_date', 'eol_tbd', 'notes')
...
class LBPhysicalAdmin(UnfoldModelAdmin):
    icon = 'router'
    list_display = ('device', 'mgmt_ip', 'model', 'version', 'environment', 'company', 'datacenter', 'monitoreo', 'netdeck')
    list_select_related = ['company', 'datacenter']
    search_fields = ('device', 'mgmt_ip', 'model', 'serial')
    list_filter = ('environment', 'company', 'datacenter', 'purpose', 'monitoreo', 'netdeck')


@admin.register(LBGuest)
class LBGuestAdmin(UnfoldModelAdmin):
    icon = 'computer'
    list_display = ('device', 'mgmt_ip', 'model', 'version', 'environment', 'company', 'monitoreo', 'netdeck')
    list_select_related = ['company']
    search_fields = ('device', 'mgmt_ip', 'model')
    list_filter = ('environment', 'company', 'monitoreo', 'netdeck')


@admin.register(LBDecommissioned)
class LBDecommissionedAdmin(ReadOnlyAdminMixin, UnfoldModelAdmin):
    """Read-only log of decommissioned LB devices. No add/change permitted."""
    icon = 'delete_forever'
    list_display = ('device', 'device_type', 'environment', 'company',
                    'decommissioned_at', 'decommissioned_by')
    list_filter = ('device_type', 'environment', 'company')
    search_fields = ('device', 'company', 'comments')
    readonly_fields = (
        'device', 'device_type', 'ci_id', 'mgmt_ip', 'version', 'model',
        'serial', 'environment', 'company', 'comments',
        'decommissioned_at', 'decommissioned_by',
    )
...
class HealthCheckDNSAdmin(UnfoldModelAdmin):
    icon = 'dns'
    list_display = ('id', 'fqdn', 'fecha', 'uptime', 'named', 'backup', 'ntp', 'company')
    search_fields = ('fqdn', 'company')
...
class SiteSettingsAdmin(SingletonAdminMixin, UnfoldModelAdmin):
    """
...
class LDAPConfigAdmin(SingletonAdminMixin, UnfoldModelAdmin):
    """
...
class LoginAuditLogAdmin(ReadOnlyAdminMixin, UnfoldModelAdmin):
    icon = 'security'
...
class ScriptRunConfigAdmin(UnfoldModelAdmin):
    ...
    Each row defines one script. Use the "Run now" action to execute
    immediately, or set a cron_schedule for automatic execution.

    The script receives today's date (localdate) formatted with date_format
    as its first positional argument:
        python /path/to/script.py 2026-04-28
    ...
    fieldsets = (
        ...
        ('Script', {
            'fields': ('script_path',),
            'description': 'Ruta absoluta al script Python que se ejecutará.',
        }),
        ('Schedule', {
            'fields': ('cron_schedule',),
            'description': (
...
class DocEntryAdmin(UnfoldModelAdmin):
    """Admin for the documentation catalog."""
...
class DirectoryEntryAdmin(UnfoldModelAdmin):
    """Admin for the important numbers directory."""
