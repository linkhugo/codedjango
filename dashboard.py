"""
Dashboard, wiki, charts, and healthcheck-chart views.

Cross-domain global search lives in ``search.py``.
"""
...
from .utils import (
    WIKI_ENVIRONMENTS, COMPLIANCE_ENVIRONMENTS, LTM_GTM_PURPOSES,
    _vip_ssl_cert_counts, _bita_avg_resolution, _bita_weekly_trend,
    get_site_settings,
)
...
def dashboard(request):
    ...
    # ── Alertas de rotación de credenciales ──────────────────────────────────
    _creds = CredentialRotation.objects.select_related('policy').all()
    _cred_overdue  = [c for c in _creds if c.status == 'overdue']
    _cred_warning  = [c for c in _creds if c.status == 'warning']

    if _cred_overdue:
        labels = ', '.join(
            f'{c.get_device_type_display()} / {c.get_credential_type_display()}'
            for c in _cred_overdue
        )
        alertas.append({
            'tipo':      'error',
            'icono':     'fa-key',
            'titulo':    'Rotación de Credenciales Vencida',
            'mensaje':   f'{len(_cred_overdue)} credencial(es) superaron su intervalo de rotación: {labels}.',
            'url':       reverse('credential_list'),
            'url_label': 'Ver Credenciales',
        })

    if _cred_warning:
        labels = ', '.join(
            f'{c.get_device_type_display()} / {c.get_credential_type_display()}'
            for c in _cred_warning
        )
        alertas.append({
            'tipo':      'warning',
            'icono':     'fa-key',
            'titulo':    'Rotación de Credenciales Próxima a Vencer',
            'mensaje':   f'{len(_cred_warning)} credencial(es) próximas a vencer: {labels}.',
            'url':       reverse('credential_list'),
            'url_label': 'Ver Credenciales',
        })
...
def wiki(request):
    ...
    # Build client_code → company name map for tooltips in the template
    company_names: dict[str, str] = dict(
        Company.objects.exclude(client_code__isnull=True).exclude(client_code='')
        .values_list('client_code', 'name')
    )
    ...
    for p in LBPhysical.objects.select_related('company').order_by('device'):
        ...
        cc = p.company.client_code if p.company else '-'
        host_tree.setdefault(env, []).append({
            ...
            'service':     p.service,
            'company':     cc,
            'company_name': company_names.get(cc, ''),
            'url_cyberark': p.url_cyberark,
        })
    ...
    for env in environments:
        ...
        env_list.append({
            ...
            'companies': [
                {
                    'name':         co,
                    'company_name': company_names.get(co, ''),
                    'devices':      sorted(devs, key=lambda d: d['device']),
                }
...
@login_required
@permission_required('lb_manager.view_healthcheckf5', raise_exception=True)
def f5_backups_chart(request):
    """Page shell — F5 Backups monthly compliance bar chart."""
    companies = (
        HealthCheckF5.objects
        .order_by('company')
        .values_list('company', flat=True)
    ...
    return render(request, 'lb_manager/f5_backups_chart.html', {'companies': companies})


@login_required
@permission_required('lb_manager.view_healthcheckf5', raise_exception=True)
def f5_backups_chart_data(request):
    """
    AJAX endpoint — daily backup compliance over LTM/GTM inventory.
    ...
    For each day of the selected month, counts how many devices from the
    inventory had a HealthCheckF5 record with file_backup IN ('OK', 'true')
    (case-insensitive). Inventory scope:
      - LBPhysical with purpose IN ('LTM','GTM') and environment IN
        ('PRE-PRODUCTION','PRODUCTION')   ←   DRP excluded
      - All LBGuest with environment IN ('PRE-PRODUCTION','PRODUCTION')
    The denominator is the static inventory size for the selected company.
    """
    def _parse_int(val):
        try:
            return int(val)
    ...
    phys_qs = LBPhysical.objects.filter(
        purpose__in=LTM_GTM_PURPOSES,
        environment__in=COMPLIANCE_ENVIRONMENTS,
    )
    guest_qs = LBGuest.objects.filter(environment__in=COMPLIANCE_ENVIRONMENTS)
    if company:
        phys_qs  = phys_qs.filter(company__name=company)
        guest_qs = guest_qs.filter(company__name=company)
    inventory_fqdns = (
        set(phys_qs.values_list('device', flat=True))
        | set(guest_qs.values_list('device', flat=True))
    )
    total_inventory = len(inventory_fqdns)

    # Two aggregated queries — no N+1:
    #   - daily_totals: días con al menos 1 healthcheck (cualquier estado)
    #   - daily_ok: días con backup OK
    base_qs = HealthCheckF5.objects.filter(
        fecha__gte=date_from, fecha__lte=date_to,
        fqdn__in=inventory_fqdns,
    )
    days_with_data = set(
        base_qs.values_list('fecha', flat=True).distinct()
    ) if inventory_fqdns else set()
    ok_filter = Q(file_backup__iexact='OK') | Q(file_backup__iexact='true')
    ok_by_day = {
        row['fecha']: row['ok']
        for row in base_qs.filter(ok_filter).values('fecha').annotate(ok=Count('id'))
    }

    days, totals, ok_list, missing_list, pct_list = [], [], [], [], []
    has_data_flags = []
    for d in range(1, num_days + 1):
        dt = dt_date(y, m, d)
        has_data = dt in days_with_data
        ok_d = ok_by_day.get(dt, 0)
        miss = max(total_inventory - ok_d, 0) if has_data else 0
        days.append(str(d))
        totals.append(total_inventory)
        ok_list.append(ok_d if has_data else 0)
        missing_list.append(miss)
        has_data_flags.append(has_data)
        if has_data and total_inventory:
            pct_list.append(round(ok_d / total_inventory * 100, 1))
        else:
            pct_list.append(None)

    ok_month      = sum(ok_list)
    missing_month = sum(missing_list)
    pct_samples   = [p for p in pct_list if p is not None]
    pct_avg_month = round(sum(pct_samples) / len(pct_samples), 1) if pct_samples else 0
    ...
    return JsonResponse({
        ...
        'pct':             pct_list,
        'has_data':        has_data_flags,
        'total_inventory': total_inventory,
        'ok_month':        ok_month,
        'missing_month':   missing_month,
        'pct_avg_month':   pct_avg_month,
    })
