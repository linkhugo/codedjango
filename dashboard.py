...
def global_search(request):
    """
    Cross-table partial IP/name search.
    ...
    using substring matching (icontains). Returns the first 25 combined results.
    Input is capped at 200 characters to prevent overly broad queries.
    """
    ...
    results = []
    if q:
        for obj in VIP.objects.filter(destination_address__icontains=q).values('ltm_fqdn', 'destination_address', 'name')[:_RESULT_LIMIT]:
            results.append({'tipo': 'VIP', 'ltm_fqdn': obj['ltm_fqdn'], 'address': obj['destination_address'], 'name': obj['name']})
        for obj in LTMNode.objects.filter(address__icontains=q).values('ltm_fqdn', 'address', 'name')[:_RESULT_LIMIT]:
            results.append({'tipo': 'LTM Node', 'ltm_fqdn': obj['ltm_fqdn'], 'address': obj['address'], 'name': obj['name']})
        for obj in SelfIP.objects.filter(address__icontains=q).values('ltm_fqdn', 'address', 'name')[:_RESULT_LIMIT]:
            results.append({'tipo': 'Self IP', 'ltm_fqdn': obj['ltm_fqdn'], 'address': obj['address'], 'name': obj['name']})
        for obj in SNATTranslation.objects.filter(snat__icontains=q).values('ltm_fqdn', 'snat', 'name')[:_RESULT_LIMIT]:
